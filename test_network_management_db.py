"""
Network Management Database Test Script
Creates 5 interconnected tables with sample data for testing DatabaseAI
"""
...
CREATE TABLE network_devices (
    ...
    hardware_id INTEGER REFERENCES hardware_info(hardware_id) ON DELETE CASCADE,
    status_id INTEGER REFERENCES device_status(status_id) ON DELETE SET NULL,
...
CREATE TABLE maintenance_logs (
    ...
    device_id INTEGER REFERENCES network_devices(device_id) ON DELETE CASCADE,
...
hardware_data = [
    ('Router', 'Cisco', 'ISR-4451', '2022-01-15', 5, 2500.00, 'TechSupply Inc'),
    ('Switch', 'HP', 'Aruba-2930F', '2022-02-20', 3, 800.00, 'NetGear Supply'),
    ('Firewall', 'Fortinet', 'FortiGate-100F', '2021-11-10', 5, 3500.00, 'SecureNet'),
    ('Access Point', 'Ubiquiti', 'UAP-AC-PRO', '2022-03-05', 2, 150.00, 'Wireless Solutions'),
    ('Server', 'Dell', 'PowerEdge-R740', '2021-08-20', 5, 5000.00, 'Dell Direct'),
    ('Router', 'Juniper', 'MX204', '2022-05-12', 5, 4200.00, 'TechSupply Inc'),
    ('Switch', 'Cisco', 'Catalyst-9300', '2022-06-18', 3, 1200.00, 'NetGear Supply'),
    ('Firewall', 'Palo Alto', 'PA-220', '2021-12-01', 5, 2800.00, 'SecureNet'),
    ('Access Point', 'Cisco', 'Meraki-MR46', '2022-07-22', 3, 600.00, 'Wireless Solutions'),
    ('Server', 'HP', 'ProLiant-DL380', '2021-09-15', 5, 4500.00, 'HP Enterprise')
]

for hw in hardware_data:
    cursor.execute("""
        INSERT INTO hardware_info (hardware_type, manufacturer, model_number, purchase_date, 
...
for status in status_data:
    cursor.execute("""
        INSERT INTO device_status (status_name, status_description, severity_level)
        VALUES (%s, %s, %s)
    """, status)
...
for i in range(10):
    ...
    install_date = (datetime.now() - timedelta(days=random.randint(365, 730))).date()
    last_maint = (datetime.now() - timedelta(days=random.randint(30, 180))).date()
    ...
    cursor.execute("""
        INSERT INTO network_devices (device_name, hardware_id, status_id, ip_address, location,
...
for i in range(15):
    ...
    maint_date = (datetime.now() - timedelta(days=random.randint(1, 365))).date()
    ...
    """, (i+1, device_id, maint_type, maint_date, tech, duration, cost,
...
for i in range(20):
    ...
    acknowledged = random.choice([True, True, False])  # 2/3 acknowledged
    resolved = acknowledged and random.choice([True, False])
    ...
    cursor.execute("""
        INSERT INTO network_alerts (device_id, alert_type, severity, alert_message, alert_timestamp,
...
print("\n" + "=" * 80)
print("COMPLEX TEST QUERIES FOR DatabaseAI")
print("=" * 80)
...
print("\n✅ Test database setup complete! You can now test DatabaseAI with these queries.")
